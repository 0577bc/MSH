...
## 2025-11-16 - 缺勤记录页面稳定性增强与写入能力

### ✅ 更新内容
- `absence-tracking.html` 统一日期键为“本地周日”，避免UTC偏移导致周日键错位。
- 增加 SDK→REST 兜底策略，网络受限时自动切HTTPS REST，避免把取数失败展示为0。
- 新增“写入本周缺勤标记”功能：按当周签到集合与成员差集，增量写入 `absenceCalc/<date>/<uuid>`。

### 🔒 合规说明
- 数据写入使用 `update()` 增量更新，符合“数据安全”规则，禁止全量覆盖。
- 未修改共享管理器与全局结构，变更范围仅限页面内部逻辑。
- 文档日期采用 `date +%Y-%m-%d`。

### 📋 后续计划
- 下周现场验证写入流程与SDK→REST兜底是否触发；如需，增加“选择任意日期写入”辅助入口。
