...
class NewDataManager {
  ...
  async checkExistingData() {
    ...
    const isGroupNamesValid = hasLocalGroupNames && typeof hasLocalGroupNames === 'object' && !Array.isArray(hasLocalGroupNames) && Object.keys(hasLocalGroupNames).length > 0;
  ...
  async performManualSync() {
    ...
    try {
      ...
      // 检查所有同步是否成功
      const allSyncSuccess = syncResults.groups && syncResults.attendanceRecords && syncResults.groupNames && syncResults.dailyNewcomers;
  ...
  verifyDataSync(localData, remoteData, dataType) {
    try {
      ...
      if (dataType === 'groups' || dataType === 'groupNames' || dataType === 'dailyNewcomers') {
        ...
        // 允许±1的差异（网络延迟可能导致）
        return Math.abs(localLength - remoteLength) <= 1;
      }
