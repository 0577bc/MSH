...
function initializeEventListeners() {
  ...
  if (cancelEditGroupButton) {
    cancelEditGroupButton.addEventListener('click', handleCancelEditGroup);
  }
      ...
      switch (dataType) {
        ...
        case 'groups':
          ...
          if (window.memberMoveInProgress || localStorage.getItem('msh_member_move_flag')) {
            console.log('⚠️ 检测到成员移动操作进行中，跳过数据同步以避免冲突');
            return;
          }
          
          // 记录远程数据时间戳（使用数据接收时间，而不是当前时间）
          const dataReceiveTime = Date.now().toString();
          localStorage.setItem('msh_groups_remote_timestamp', dataReceiveTime);
          ...
          if (localGroups) {
            try {
              ...
              console.log('管理页面 - 使用本地groups数据，确保数据不丢失（包括花名）');
            ...
            console.log('管理页面 - 没有本地数据，使用远程groups数据');
            groups = data;
          }
