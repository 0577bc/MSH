# [2025-11-13] - 事件管理外部表单一体化

### ✨ 新增功能
- **事件操作栏增强**：`absence-events.html` 新增“抓取”“删除外部事件”按钮，自动识别最新回访与外部状态，按钮状态实时反馈。
- **批量操作控件**：添加“一键转发 / 一键抓取 / 一键删除外部事件”批量按钮，按事件状态自动计算待处理数量，支持进度提示与失败统计。

### 🔄 外部系统联动
- **外部表单抓取**：兼容 `GET /api/events/lookup` 最新返回格式，抓取回访后写回 `absenceEvents`/`trackingRecords` 并同步 localStorage。
- **外部事件删除**：对接 `DELETE /api/events/byExternalId/:absenceEventId`，删除成功或外部不存在时自动标记 `externalDeleted`，保证 UI 与 Firebase 状态一致。

### 🛡️ 稳定性优化
- 扩展单条操作函数以支持静默模式、跳过刷新、异常抛出等批量策略；增加节流延迟，避免外部 API 触发限流。
- 全面调试输出请求/响应日志，方便联调定位；批量流程结束后统一刷新并更新按钮数量。

### 📅 2025-11-16 - 缺勤记录页面增强与合规检查
- **absence-tracking.html**
  - 修复周日日期键使用UTC导致的错位，统一为“本地时区周日”键值。
  - 新增 SDK→REST 兜底：SDK握手失败或返回空时自动切HTTPS REST，避免误报“缺勤=0”。
  - 新增“写入本周缺勤标记”按钮：按当周签到集合与成员差集，为缺勤成员增量写入 `absenceCalc/<YYYY-MM-DD>/<uuid>`（使用 `update()`，禁止 `set()` 全量覆盖）。
  - 失败态显示“数据加载失败”，不再把网络失败当作 0。
- **Vercel 配置**：补齐 `externalFormConfig.features` 与外部端点，避免“功能未启用”误报。
- **规则合规**：
  - 遵循“数据安全”规则：缺勤标记写入使用 `update()` 增量更新；无任何 `.set()` 全量覆盖。
  - 遵循“共享函数修改保护”：未修改 `new-data-manager.js`、`utils.js` 或全局结构，仅在页面内新增独立函数；功能前已给出分析并征得确认。
  - 遵循“时间管理”：文档日期基于 `date +%Y-%m-%d`。
