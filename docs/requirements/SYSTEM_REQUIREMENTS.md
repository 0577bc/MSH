...
### 1. 事件管理与个人跟踪
- **absence-events.html**：统一的事件管理页面（当前主用）
  - 管理缺勤事件状态、转发外部表单、关闭事件
- **personal-page.html**：成员维度的个人主日跟踪视图
  - 支持按成员查看所有事件及回访记录
  - 提供测试态的手动回访录入入口
- **缺勤事件生成**：通过 `tools/msh-system/absence-calculator.html` 批量计算连续缺勤结果，并写回 `absenceEvents`、`trackingRecords`
- **历史页面**：原 `sunday-tracking.html` 已退出使用；旧的自动生成逻辑已彻底退役，`SundayTrackingManager` 仅负责读取、同步及回访更新
...
- **特性**:
  ...
  - **智能合并**: 三路合并算法处理数据冲突
  - **实例复用**: 避免重复初始化和数据拉取
  - **数据预加载**: 缺勤事件由批量工具生成，NewDataManager 负责缓存和同步最新结果
