...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (signinButton) {
    signinButton.addEventListener('click', async () => {
      ...
      // 使用简化的数据引用管理器
      const success = window.utils.DataReferenceManager.attendanceRecords.add(record);
  ...
  if (saveNewMemberButton && newGroupSelect && newMemberName && newMemberPhone) {
    saveNewMemberButton.addEventListener('click', async () => {
      ...
      if (window.utils && window.utils.generateMemberId) {
        newMember.id = window.utils.generateMemberId(group, newMember.name);
      } else {
        // 简单的ID生成逻辑
        const prefix = getGroupPrefix(group);
        const memberCount = groups[group] ? groups[group].length : 0;
        newMember.id = prefix + String(memberCount + 1).padStart(3, '0');
      }
